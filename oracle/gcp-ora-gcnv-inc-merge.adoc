...
---
= TR-5007: Otimize o backup e a recuperação do Oracle VLDB com mesclagem incremental no servidor em Google Cloud NetApp Volumes
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Recuperar um banco de dados muito grande (VLDB) no Oracle usando a ferramenta de backup Oracle Recovery Manager (RMAN) pode ser uma tarefa altamente desafiadora.  O processo de restauração do banco de dados a partir da mídia de backup em caso de falha pode ser demorado, atrasando a recuperação do banco de dados e potencialmente impactando significativamente seu Contrato de Nível de Serviço (SLA).  A situação está piorando ainda mais para o Oracle VLDB implantado na nuvem, pois o backup RMAN geralmente é transmitido para o armazenamento de objetos devido a considerações de custo.  Como resultado, não é viável restaurar e recuperar um VLDB se o SLA for essencial.  A partir da versão 10g, a Oracle introduziu um recurso RMAN que permite aos usuários criar cópias de imagem preparadas dos arquivos de dados do banco de dados Oracle em armazenamento em disco adicional localizado no host do servidor de banco de dados.  Essas cópias de imagem podem ser atualizadas incrementalmente usando o RMAN em um intervalo definido pelo usuário.  Em caso de falha, o administrador do banco de dados (DBA) pode alternar rapidamente o banco de dados Oracle da mídia com falha para a cópia de imagem, eliminando a necessidade de uma restauração completa da mídia do banco de dados.  O resultado é um SLA muito melhorado, embora ao custo de dobrar o armazenamento de banco de dados necessário.

Se você estiver interessado no SLA para seu VLDB e pensando em mover o banco de dados Oracle para uma nuvem pública, como o Google Cloud Platform (GCP), poderá configurar uma estrutura de proteção de banco de dados semelhante usando recursos como o Google Cloud NetApp Volumes (GCNV) para preparar sua cópia de imagem do banco de dados em espera.  O GCNV é um serviço de armazenamento de arquivos totalmente gerenciado e de alto desempenho oferecido pelo Google que torna mais fácil, seguro e econômico migrar e executar aplicativos empresariais exigentes, como o banco de dados Oracle, no Google Cloud.  Seus variados níveis de serviço permitem a flexibilidade de controlar o custo de armazenamento, ao mesmo tempo em que atingem um RPO próximo de zero e um RTO gerenciável.  Nesta documentação, demonstramos como provisionar e exportar um sistema de arquivos NFS de um pool de armazenamento GCNV para ser montado em um servidor de banco de dados Oracle para preparar uma cópia de banco de dados em espera para recuperação rápida em caso de falha de armazenamento primário.
...
O teste e a validação desta solução foram realizados em pools de armazenamento do Google Cloud NetApp Volumes e em VMs do Google Compute Engine que podem não corresponder ao ambiente de implantação final. Para mais informações, consulte a seção <<Fatores-chave para consideração de implantação>> .



=== Arquitetura

image:gcp-ora-gcnv-inc-merge-architecture.png["Esta imagem fornece uma visão detalhada da implementação de mesclagem incremental do Oracle VLDB no Google Cloud com GCNV."]
...
| VM do Google Compute Engine para servidor de banco de dados | N1 (4 vcpus, 15 GiB de memória) | 2 VMs, uma como servidor de banco de dados primário e a outra como servidor de banco de dados de espera ou clone 
...
* *Nível de serviço do pool de armazenamento GCNV.*  O pool de armazenamento GCNV oferece quatro níveis de serviço: Standard, Premium, Extreme e Flex.  Por padrão, um QoS automático é aplicado a um volume criado dentro de um pool de armazenamento, o que restringe a taxa de transferência no volume com base em seu tamanho e no nível de serviço.  A única exceção é que os volumes alocados do pool de armazenamento Flex compartilham a taxa de transferência disponível para o pool de armazenamento.
* *Limite de armazenamento de backup no servidor Oracle VLDB para mesclagem incremental RMAN.*  Em nosso teste e validação, um volume NFS para backup incremental e mesclagem do Oracle é alocado de um único pool de armazenamento GCNV com nível de serviço padrão, que tem um limite de capacidade de 200 TiB.  Para implantação acima desses limites, vários pools de armazenamento GCNV podem ser implementados em paralelo com vários pontos de montagem NFS para fornecer maior capacidade.
* *Recuperação do Oracle usando mesclagem incremental do RMAN.*  O backup incremental e a mesclagem no servidor RMAN geralmente são executados em uma frequência definida pelo usuário com base em seus objetivos de RTO e RPO.  Se houver perda total do armazenamento do arquivo de banco de dados primário e/ou logs arquivados, poderá ocorrer perda de dados.  O banco de dados Oracle pode ser recuperado até o último backup incremental disponível a partir da cópia da imagem de backup do banco de dados no servidor.  Para minimizar a perda de dados, a área de recuperação flash do Oracle precisa ser configurada no ponto de montagem de backup NFS do GCNV, e os logs arquivados são copiados para a mesma montagem NFS junto com a cópia da imagem do banco de dados para avançar o banco de dados para o último log arquivado, no caso de uma falha.  Dessa forma, é possível atingir um RPO próximo de zero.
* *Executando Oracle VLDB a partir de backup de imagem NFS no pool de armazenamento GCNV.*  Neste ponto, não há suporte para reatribuir o nível de serviço de volume do banco de dados de padrão para premium ou extremo.  No entanto, a reatribuição de níveis de serviço de premium para extremo é suportada.  Para executar o banco de dados em uma cópia de imagem por um longo período após uma falha, é recomendável alocar um volume do pool de armazenamento premium para backup de cópia de imagem do banco de dados.
* *VMs do Google Compute Engine.*  No teste e validação, usamos o tipo N1 de instância de VM do GCE como servidores de banco de dados Oracle.  Existem outros tipos de VMs do GCE que podem ser mais otimizadas e mais adequadas para a carga de trabalho do banco de dados.  Dimensione a VM adequadamente para o número de vCPUs e a quantidade de RAM com base nos requisitos reais da carga de trabalho.
* *Configuração do dNFS.* O dNFS é integrado ao kernel do Oracle e é conhecido por aumentar drasticamente o desempenho do banco de dados Oracle quando o Oracle é implantado no armazenamento NFS. O dNFS é empacotado no binário do Oracle, mas não é ativado por padrão.  Ele deve ser ativado para qualquer implantação de banco de dados Oracle no NFS.  Para a implantação de vários pools de armazenamento GCNV para um VLDB, os múltiplos caminhos dNFS para diferentes pools de armazenamento GCNV devem ser configurados corretamente.
...
A solução pressupõe que um Oracle VLDB já esteja implantado no ambiente do Google Cloud dentro de uma VPC.  O Oracle VLDB pode ser executado no armazenamento GCNV ou em qualquer outra opção de armazenamento no ecossistema do Google Cloud.  A seção a seguir fornece procedimentos de implantação passo a passo para configurar um backup de imagem Oracle VLDB e mesclagem incremental via RMAN para um sistema de arquivos NFS montado a partir de um pool de armazenamento GCNV.  O Oracle VLDB também tem seu armazenamento primário hospedado em um pool de armazenamento GCNV diferente.
...
. No console do portal do Google Cloud, siga a documentaçãolink:https://cloud.google.com/netapp/volumes/docs/configure-and-use/storage-pools/create-storage-pool["Criar um pool de armazenamento"^] para criar um pool de armazenamento GCNV com serviço padrão, conforme mostrado abaixo.
+
image:gcp-ora-gcnv-inc-merge-pool-001.png["Esta imagem mostra a tela do pool de armazenamento GCNV do console do portal do Google Cloud"]

. No console do portal do Google Cloud, siga a documentaçãolink:https://cloud.google.com/netapp/volumes/docs/configure-and-use/volumes/create-volume["Criar um volume"^] para criar um volume para backup de imagem Oracle VLDB no servidor.  Nesta demonstração de solução, o armazenamento primário do Oracle VLDB também é hospedado no GCNV, conforme mostrado abaixo.
+
image:gcp-ora-gcnv-inc-merge-vol-002.png["Esta imagem mostra a tela de volumes de armazenamento GCNV do console do portal do Google Cloud"]
...
+
image:gcp-ora-gcnv-inc-merge-vol-003.png["Esta imagem mostra a tela de volumes de armazenamento GCNV do console do portal do Google Cloud"]

. Efetue login no servidor Oracle VLDB primário.  Como root, monte o volume NFS exportado do armazenamento GCNV.  Altere o endereço IP do seu servidor NFS e o caminho do arquivo conforme necessário.
+
[source, cli]
----
sudo mkdir /nfsgcnv
...
. Para esta validação da solução, o banco de dados primário é configurado como um servidor autônomo com infraestrutura de grade. Reinicie a configuração conforme mostrado abaixo.  O armazenamento primário é hospedado no pool de armazenamento GCNV com serviço premium.  Novamente, isso não é um requisito e o armazenamento principal pode estar em qualquer outra opção de armazenamento no Google Cloud.
+
....
...
....
. Liste a cópia da imagem do banco de dados após o backup para observar se uma cópia da imagem do banco de dados foi criada no ponto de montagem do GCNV NFS.
+
....
...
[%collapsible%open]
====
No caso de uma falha devido a um problema de armazenamento primário, como perda ou corrupção de dados, o banco de dados pode ser rapidamente alternado para uma cópia de imagem na montagem GCNV NFS e recuperado para o estado atual sem restauração do banco de dados.  Eliminar a restauração de mídia acelera tremendamente a recuperação do banco de dados para um VLDB.  Este caso de uso pressupõe que o servidor Oracle VLDB DB esteja intacto e que o arquivo de controle do banco de dados, os logs arquivados e os logs atuais estejam todos disponíveis para recuperação.
...
....
. Insira uma linha na tabela de teste.  A linha é inserida após um backup incremental e mesclagem mais recente e seus dados não estão no conjunto de backup.  A demonstração subsequente mostraria que a linha de teste é recuperada depois que o banco de dados é completamente recuperado.
+
....
...
....
. Verifique a estrutura do banco de dados do sqlplus após a recuperação para observar que todos os arquivos de dados VLDB, com exceção dos arquivos de controle, temporários e de log atuais, agora estão alternados para cópia no sistema de arquivos GCNV NFS.
+
....
...
[%collapsible%open]
====
Após a falha de armazenamento no armazenamento primário ser corrigida, o banco de dados pode ser facilmente revertido para o armazenamento primário com tempo de inatividade mínimo.  A seguir estão os procedimentos passo a passo.
...
....
. Revise o script de backup e mesclagem do RMAN para apontar para o armazenamento primário e, em nosso caso de teste, o grupo de discos ASM +DATA na definição de canal do RMAN.  Execute o script de backup revisado para criar uma nova cópia de imagem no armazenamento primário.
+
....
...
[%collapsible%open]
====
No caso de uma falha em que o armazenamento primário e/ou o host do servidor de banco de dados primário sejam perdidos, todas as transações que não estiverem nos arquivos de log arquivados serão perdidas.  No entanto, a cópia da imagem de backup do banco de dados Oracle disponível no sistema de arquivos GCNV NFS é útil.  Você pode recuperar rapidamente o banco de dados primário para o mesmo servidor de banco de dados primário ou para um servidor de banco de dados standby, se houver um disponível, usando a cópia de imagem de backup.  Nesta seção, demonstraremos os procedimentos passo a passo para recuperação no servidor de banco de dados primário.  Os métodos para recuperar para um novo servidor de banco de dados em espera são quase idênticos; no entanto, a cópia da imagem de backup do banco de dados deve ser montada no servidor de banco de dados em espera com a mesma pilha de software Oracle instalada e configurada.
...
. Se a recuperação for executada em um servidor de banco de dados em espera, certifique-se de que seja o mesmo sistema operacional, a mesma versão e o mesmo kernel do sistema operacional que foram corrigidos no host do servidor de banco de dados principal.  Além disso, a mesma versão e patches do Oracle foram instalados e configurados no servidor de banco de dados em espera com opção somente software.
...
. A cópia da imagem de backup do banco de dados Oracle no sistema de arquivos GCNV NFS é então montada no servidor de banco de dados em espera para recuperação.  Os procedimentos a seguir demonstram os detalhes do processo.
+
...
----
. Verifique os logs arquivados do Oracle disponíveis na montagem GCNV NFS para recuperação e anote o último número de sequência de log do arquivo de log.  No nosso caso de teste, foi 27.  Nosso ponto de recuperação é até a sequência de log número 28.
. Se estiver recuperando de um host de banco de dados em espera, como usuário Oracle, defina a variável ORACLE_HOME como a instalação atual do Oracle no servidor de banco de dados em espera e o ORACLE_SID como o SID da instância primária do Oracle.  Neste teste, é NTAP.
+
....
...
....
. Como usuário Oracle, crie um arquivo init genérico do Oracle no diretório $ORACLE_HOME/dbs com os diretórios de administração apropriados configurados.  O mais importante é ter o Oracle `flash recovery area` aponte para o caminho de montagem GCNV NFS conforme definido no servidor Oracle VLDB primário.  `flash recovery area` a configuração é demonstrada na seção `Setup Oracle RMAN incremental merge to image copy on GCNV for in-server VLDB backup` .  Defina o arquivo de controle do Oracle para o sistema de arquivos de backup GCNV NFS.
+
[source, cli]
----
...
. Como usuário Oracle, inicie o RMAN para executar a recuperação do Oracle no host do servidor de banco de dados primário ou em espera.  Primeiro, inicie a instância do Oracle em `nomount` estado.
+
....
...
....
. Definir ID do banco de dados.  O ID do banco de dados pode ser recuperado do nome do arquivo Oracle da cópia da imagem no ponto de montagem GCNV NFS.
+
....
...
....
. Para retornar ao armazenamento primário, siga exatamente os mesmos procedimentos detalhados na seção `Reverse VLDB back to primary storage from backup copy` .  Se estiver recuperando em um servidor de banco de dados em espera, o armazenamento primário precisará ser importado para o servidor de banco de dados em espera.  Também é necessário realocar os arquivos de controle do Oracle e os arquivos de log on-line atuais da montagem GCNV nfs de volta para o armazenamento primário, o que pode ser feito em poucos minutos de inatividade.
...
[%collapsible%open]
====
Clonar um novo Oracle VLDB a partir da cópia de imagem de backup no GCNV é semelhante à recuperação pontual do banco de dados primário para um novo servidor de BD.  A equipe de engenharia da NetApp Solutions fornece um kit de ferramentas de automação para facilitar todos os processos para uma implementação rápida (minutos) e sem esforço.

O kit de ferramentas de automação a seguir contém um manual que pode ser usado para criar ou atualizar clones de um Oracle VLDB a partir de uma cópia de imagem de backup do banco de dados primário armazenada no armazenamento GCNV por meio de montagem NFS para um gerenciamento completo do ciclo de vida do clone.  Para mais detalhes, consulte a documentação:link:automation-ora-bkup-clone.html["Ciclo de vida automatizado do clone Oracle VLDB com backup de imagem no servidor"] .
