...
summary: 'A solução fornece um kit de ferramentas de automação baseado em Ansible para configurar, clonar e atualizar um Very Large Database (VLDB) clonado da Oracle a partir de uma cópia de backup de imagem de banco de dados Oracle residente em volumes de nuvem NetApp , que estão disponíveis em uma plataforma de nuvem pública.  O kit de ferramentas inicialmente oferece suporte ao Google Cloud NetApp Volumes (GCNV), mas pode ser expandido para oferecer suporte a outros volumes de nuvem, como Azure NetApp Files (ANF) ou Amazon FSx for ONTAP (FSxN).' 
---
= Ciclo de vida automatizado do clone Oracle VLDB com backup de imagem no servidor
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Aproveitar o recurso de clonagem rápida (minutos) dos volumes NetApp ONTAP para Oracle VLDB a partir de uma cópia de backup de imagem de banco de dados no servidor na nuvem pública atende a vários propósitos.  Ele fornece um banco de dados de produção completo e quase em tempo real para relatórios, desenvolvimento ou casos de uso de UAT.  Ele economiza em custos de armazenamento, principalmente quando uma clonagem fina dos volumes de backup do banco de dados primário é uma opção.  Este kit de ferramentas de automação baseado em Ansible permite que os usuários configurem, clonem e atualizem bancos de dados Oracle clonados de acordo com os cronogramas desejados pelo usuário para um gerenciamento simplificado do ciclo de vida.  O kit de ferramentas é destinado ao caso de uso do Oracle VLDB implantado na nuvem pública do Google e emprega GCNV como armazenamento de backup de banco de dados no servidor.

Esta solução aborda os seguintes casos de uso:

* Configure arquivos de configuração de clone de banco de dados para clonagem automatizada de banco de dados Oracle.
* Crie ou atualize um banco de dados Oracle clone a partir de uma cópia de backup de imagem de banco de dados primário usando um manual do Ansible em um agendador.
...
* Um proprietário de aplicativo que gosta de clonar bancos de dados Oracle a partir de uma cópia de backup de imagem de banco de dados para outros casos de uso.
...
NOTE: Para simplificar, o servidor Oracle de destino do clone deve ser configurado de forma idêntica ao servidor Oracle primário, como a pilha de software Oracle, bem como o layout do diretório para o Oracle Home, etc.
...
[%collapsible%open]
====
O kit de ferramentas inclui um arquivo de hosts que define os hosts Oracle de origem e de destino nos quais o playbook do Ansible é executado.  Normalmente, ele inclui o host do servidor de banco de dados principal e o host do servidor de banco de dados clone de destino.  A seguir está um arquivo de exemplo.  Uma entrada de host inclui o endereço IP do host de destino, bem como a chave SSH para acesso do usuário ao host para executar comandos de clonagem ou atualização.  O armazenamento do Google Cloud NetApp Volumes é acessado e gerenciado via gcloud cli.
...
[%collapsible%open]
====
O servidor Oracle de destino clone deve ter a mesma pilha de software Oracle que o servidor Oracle de origem instalado e corrigido.  A variável $ORACLE_HOME idealmente deve corresponder à configuração do servidor Oracle de origem.  Se a configuração do ORACLE_HOME de destino for diferente da configuração do servidor Oracle principal, crie um link simbólico para contornar as diferenças.

Se o banco de dados primário estiver configurado com ASM, o grupo primário de arquivos de dados poderá pertencer ao grupo ASM e o mesmo grupo ASM com o mesmo ID de grupo deverá ser adicionado ao host clone para evitar problemas de permissão.
...
Para clonar quaisquer bancos de dados adicionais, crie um oracle_bkup_clone_n_gcnv.yml e um oracle_bkup_clone_n_gcnv.sh separados.  Configure os hosts de destino do Ansible, o arquivo global vars.yml e o arquivo hostname.yml no diretório host_vars adequadamente.
...
=== Detalhes da execução do manual

[%collapsible%open]
====
O arquivo de log a seguir captura os detalhes da saída de uma execução completa do manual para referência.
