...
summary: A solução fornece uma visão geral e detalhes para configurar o Oracle Data Guard usando o AWS FSx ONTAP como armazenamento de banco de dados Oracle em site standby para reduzir o licenciamento e o custo operacional da solução Oracle Data Guard HA/DR na AWS. 
---
...
image:aws-ora-fsx-data-guard-architecture.png["Esta imagem fornece uma visão detalhada da implementação do Oracle Data Guard na AWS com FSx ONTAP."]
...
| Instância EC2 para computação | t2.xlarge/4vCPU/16G | Três instâncias EC2 T2 xlarge EC2, uma como servidor de banco de dados primário, uma como servidor de banco de dados em espera e a terceira como servidor de banco de dados clone 
...
| Standby físico | db1_LA | db1_LA.demo.netapp.com 
|===
...
....
. Reinicie o banco de dados em espera em `mount` prepare e execute o seguinte comando para ativar a recuperação gerenciada do banco de dados em espera.
+
[source, cli]
----
...
A seguir estão os procedimentos de alto nível para clonar um banco de dados de LEITURA/GRAVAÇÃO do banco de dados de espera físico gerenciado no Data Guard usando o SnapCenter.  Para obter instruções detalhadas sobre como configurar e configurar o SnapCenter, consultelink:../dbops/hybrid-dbops-snapcenter-usecases.html["Soluções de banco de dados em nuvem híbrida com SnapCenter"^] seções relevantes do Oracle.
...
....
. Adicionar cluster de armazenamento FSx a `Storage Systems` no SnapCenter com IP de gerenciamento de cluster FSx e credencial fsxadmin.
+
image:aws-ora-fsx-data-guard-clone-001.png["Captura de tela mostrando esta etapa na GUI."]

. Adicionar AWS ec2-user a `Credential` em `Settings` .
+
image:aws-ora-fsx-data-guard-clone-002.png["Captura de tela mostrando esta etapa na GUI."]

. Adicionar instância de banco de dados EC2 em espera e clonar instância de banco de dados EC2 para `Hosts` .
+
image:aws-ora-fsx-data-guard-clone-003.png["Captura de tela mostrando esta etapa na GUI."]

+

NOTE: A instância do EC2 DB clone deve ter pilhas de software Oracle semelhantes instaladas e configuradas.  Em nosso caso de teste, a infraestrutura de grade e o Oracle 19C foram instalados e configurados, mas nenhum banco de dados foi criado.

. Crie uma política de backup personalizada para backup completo do banco de dados offline/montado.
+
image:aws-ora-fsx-data-guard-clone-004.png["Captura de tela mostrando esta etapa na GUI."]

. Aplicar política de backup para proteger o banco de dados em espera em `Resources` aba.
+
image:aws-ora-fsx-data-guard-clone-005.png["Captura de tela mostrando esta etapa na GUI."]

. Clique no nome do banco de dados para abrir a página de backups do banco de dados.  Selecione um backup a ser usado para clonagem do banco de dados e clique em `Clone` botão para iniciar o fluxo de trabalho de clonagem.
+
image:aws-ora-fsx-data-guard-clone-006.png["Captura de tela mostrando esta etapa na GUI."]

. Selecione `Complete Database Clone` e nomeie o SID da instância do clone.
+
image:aws-ora-fsx-data-guard-clone-007.png["Captura de tela mostrando esta etapa na GUI."]
...
+
image:aws-ora-fsx-data-guard-clone-008.png["Captura de tela mostrando esta etapa na GUI."]
...
+
image:aws-ora-fsx-data-guard-clone-009.png["Captura de tela mostrando esta etapa na GUI."]
...
+
image:aws-ora-fsx-data-guard-clone-010.png["Captura de tela mostrando esta etapa na GUI."]
...
+
image:aws-ora-fsx-data-guard-clone-011.png["Captura de tela mostrando esta etapa na GUI."]

. Configure a notificação por e-mail, se desejar.
+
image:aws-ora-fsx-data-guard-clone-012.png["Captura de tela mostrando esta etapa na GUI."]

. Revise o resumo, clique `Finish` para iniciar o clone.
+
image:aws-ora-fsx-data-guard-clone-013.png["Captura de tela mostrando esta etapa na GUI."]

. Monitorar trabalho de clonagem em `Monitor` aba.  Observamos que demorava cerca de 8 minutos para clonar um banco de dados com cerca de 300 GB de volume.
+
image:aws-ora-fsx-data-guard-clone-014.png["Captura de tela mostrando esta etapa na GUI."]
...
+
image:aws-ora-fsx-data-guard-clone-015.png["Captura de tela mostrando esta etapa na GUI."]
...
+
link:aws-ora-fsx-ec2-deploy-intro.html["Introdução"]
